"""Backfill detailed audit-report sections from DART audit submission attachments.

This runner intentionally targets listed-company "감사보고서제출" filings. Those
filings expose detailed audit reports through DART web attachment viewer dcmNo
pages, so the backfill can run without requiring every external MCP user to have
a DART API key.
"""
...
def main() -> int:
    ...
    logging.info(
        "audit submission backfill start start_year=%s end_year=%s market=%s targets=%s",
        args.start_year,
        args.end_year,
        args.market or "ALL",
        len(targets),
    ...
    ok = failed = sections = documents = 0
    failure_outcomes: list[str] = []
    for idx, target in enumerate(targets, 1):
        started = datetime.utcnow()
        try:
            result = collect_report_sections_for_disclosure(target["rcept_no"])
            if result.get("ok"):
                ok += 1
        ...
        except Exception as exc:  # keep long-running backfill alive
            failed += 1
    ...
    logging.info(
        "audit submission backfill done targets=%s ok=%s failed=%s documents=%s sections=%s",
        len(targets),
        ok,
        failed,
        documents,
