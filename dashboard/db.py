...
주: Streamlit import는 headless 환경(MCP 서버, 배치 작업)에서도 이 모듈을 사용할 수 있도록
optional 처리한다. 환경변수 DART_HEADLESS=1이 설정되어 있거나 streamlit이 미설치면
@st.cache_data는 no-op 스텁으로 대체된다.
"""
import os
import sys
...
class _HeadlessST:
    """streamlit 미설치/headless 환경용 스텁. @st.cache_data만 no-op 지원."""

    @staticmethod
    def cache_data(*args, **kwargs):
        # 1) @st.cache_data (bare, no args): args[0]가 함수
        if args and callable(args[0]) and not kwargs:
            return args[0]

        # 2) @st.cache_data(ttl=...) 등: decorator factory 반환
        def _decorator(func):
            return func
...
_FORCE_HEADLESS = os.environ.get("DART_HEADLESS", "").lower() in ("1", "true", "yes")
