...
1. `get_business_overview` with `include_semantic_context=true` for cached
   DART business report, audit report, note, disclosure, and financial evidence;
2. one customizable `peer_criteria` cohort selection; and
3. `compare_peer_accounting_policies` with `include_note_comparison=true`,
   which exposes the same side-by-side note evidence without resolving another
   public tool or peer universe.
...
Use `get_business_overview` with `include_semantic_context=true` or
`compare_peer_accounting_policies` with `include_note_comparison=true` when
only a single local-DART surface is needed. Their local-cache limitations and
provenance remain visible; neither call obtains IR or web/news evidence.
