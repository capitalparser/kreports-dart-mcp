...
_DESCRIPTIONS = {
    ...
    "search_dataset": "주요 로컬 캐시 데이터셋을 회사, 연도, 시장, 업종, 키워드 기준으로 공통 검색한다. accounting_note_chapters를 검색하면 주제별로 해당 정보를 공시한 회사 매트릭스를 반환하며, financial_metric·financial_min/max·financial_year로 자산·매출·이익 등 재무 매트릭스 조건을 추가할 수 있다. 감사보고서 섹션(KAM/강조/기타/계속기업), 회계정책, 주석, 공시목록, 감사보수·시간, 재무요약을 동일한 응답 구조로 반환하여 '어느 회사/업종/연도에 해당 이슈가 있는가' 유형의 질문에 사용한다. 로컬 캐시 일치는 공시 부재 결론이 아니다.",
