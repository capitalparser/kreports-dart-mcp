...
_DESCRIPTIONS = {
    ...
    "get_business_overview": "사업보고서 핵심 경영 정보 텍스트 + 업종 특화 인사이트 반환. 사업개요, 사업내용, 위험관리, 경영계획, R&D, 주요계약 6개 섹션의 본문 텍스트와 KSIC 업종 분류 기반 rule-based 인사이트(R&D 비율, 위험 분포, 업종 키워드)를 포함. include_semantic_context=true이면 같은 회사·연도의 로컬 DART 증빙 버킷과 source locator를 추가하며, 캐시 미확보는 공시 부재가 아니다. 읽기 전용이며 DART API 호출이나 백필을 수행하지 않는다.",
    ...
    "compare_peer_accounting_policies": "회계정책 주석 peer 비교. 기본 호출은 대상회사 정책 캐시와 peer item_key 커버리지를 보여준다. item_key 또는 keyword를 지정하면 대상회사와 최종 peer의 heading·제한된 본문 excerpt를 나란히 비교한다. include_note_comparison=true이면 수익, 리스, 금융상품, 특수관계자, 충당·우발, 손상, 종속기업, 후속사건, 회계정책 topic별 원문 excerpt·source locator·외부화 상태를 같은 사업연도 peer별로 추가한다. 로컬 캐시 부재는 unavailable이며 공시 부재가 아니다. 후보군은 adaptive KSIC prefix와 sector 필터만 사용하며 market은 표시용, business text는 미색인이다. auditor/investor/balanced 선정 프로필, 안전한 재무 가중치·규모범위, 명시적 peer 포함/제외를 지원한다. 재무 유사도는 접수번호로 입증되지 않은 내부 캐시 스크리닝 입력이며 가중치는 감사·회계 기준이 아니다. 정확히 검증된 해당 회사·연도 사업보고서 접수번호만 DART 링크로 제공한다.",
    ...
    "search_dataset": "주요 로컬 캐시 데이터셋을 회사, 연도, 시장, 업종, 키워드 기준으로 공통 검색한다. accounting_note_chapters를 검색하면 주제별로 해당 정보를 공시한 회사 매트릭스를 반환하며, financial_metric·financial_min/max·financial_year·financial_quarter로 자산·매출·이익 등 재무 매트릭스 조건을 추가할 수 있다. 금액 경계와 반환값은 저장 원화(KRW), Beneish_M 점수는 dimensionless이다. 감사보고서 섹션(KAM/강조/기타/계속기업), 회계정책, 주석, 공시목록, 감사보수·시간, 재무요약을 동일한 응답 구조로 반환하여 '어느 회사/업종/연도에 해당 이슈가 있는가' 유형의 질문에 사용한다. 로컬 캐시 일치는 공시 부재 결론이 아니다.",
...
_INPUT_MODELS = {
    ...
    "prepare_standard_audit_hours_inputs": models.PrepareStandardAuditHoursInputsInput,
    "prepare_audit_materiality_inputs": models.PrepareAuditMaterialityInputsInput,
