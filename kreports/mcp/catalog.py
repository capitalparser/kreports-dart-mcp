...
_DESCRIPTIONS = {
    ...
    "compare_peer_accounting_policies": "회계정책 주석 peer 비교. 기본 호출은 대상회사 정책 캐시와 peer item_key 커버리지를 보여준다. item_key 또는 keyword를 지정하면 대상회사와 최종 peer의 heading·제한된 본문 excerpt를 나란히 비교한다. auditor/investor/balanced 선정 프로필, 안전한 재무 가중치·규모범위, 명시적 peer 포함/제외를 지원하며 선정 근거와 캐시 누락을 구분한다. 가중치는 내부 스크리닝 휴리스틱이며 감사·회계 기준이 아니다. 정확히 검증된 해당 회사·연도 사업보고서 접수번호만 DART 링크로 제공한다.",
