"""Strict typed arguments for 33 public and one operator-opt-in MCP tool."""
...
class SearchDatasetInput(ToolInput):
    ...
    financial_min: float | None = Field(
        None,
        description="선택. financial_metric 하한(포함). 금액 지표는 저장 원화(KRW), Beneish_M 점수는 무차원 값.",
    )
    financial_max: float | None = Field(
        None,
        description="선택. financial_metric 상한(포함). 금액 지표는 저장 원화(KRW), Beneish_M 점수는 무차원 값.",
    )
    financial_year: Year | None = Field(
        None,
        description="선택. 재무 필터 사업연도. 생략 시 year를 사용하고, 둘 다 없으면 최신값이 아닌 명시적 연도 필요.",
    )
    financial_quarter: int = Field(
        4,
        ge=1,
        le=4,
        description="재무 필터 분기. 기본 4분기(연간 비교 기준).",
    )
    ...
    def validate_financial_filter(self):
        ...
        if self.financial_min is not None and not math.isfinite(self.financial_min):
            raise ValueError("financial_min은 유한한 숫자여야 합니다.")
        if self.financial_max is not None and not math.isfinite(self.financial_max):
            raise ValueError("financial_max는 유한한 숫자여야 합니다.")
