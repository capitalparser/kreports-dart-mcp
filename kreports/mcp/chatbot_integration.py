...
def _chatbot_enrich(
    ...
    assert _BASE_ENRICH is not None
    enriched = _BASE_ENRICH(tool_name, result)
...
def install_chatbot_enrichment() -> None:
    """Patch both contract and dispatch references exactly once."""
    global _INSTALLED, _BASE_ENRICH
    if _INSTALLED:
        return

    from kreports.mcp import contracts as contracts_module
    from kreports.mcp import dispatch as dispatch_module

    _BASE_ENRICH = contracts_module.enrich_answer_response
    contracts_module.enrich_answer_response = _chatbot_enrich
    dispatch_module.enrich_answer_response = _chatbot_enrich
    _INSTALLED = True