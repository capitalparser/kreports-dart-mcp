...
from kreports.mcp.chatbot_integration import install_chatbot_enrichment
...
class EnhancedComparePeerAccountingNotesInput(
    models.ComparePeerAccountingNotesInput
):
    fs_basis_policy: Literal[
        "fallback_with_warning",
        "strict",
    ] = Field(
        "fallback_with_warning",
        description=(
            "strict이면 cohort의 CFS/OFS 기준과 다른 주석을 "
            "비교에서 제외한다. 기본값은 fallback을 표시한 채 허용한다."
        ),
    )
...
_EXTENSIONS = {
    ...
    "compare_peer_accounting_notes": (
        EnhancedComparePeerAccountingNotesInput
    ),
...
def install_catalog_extensions() -> None:
    """Install richer schemas idempotently while keeping 34 public tools."""
