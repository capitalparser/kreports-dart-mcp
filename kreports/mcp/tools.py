...
def _handle_get_accounting_policy_changes(args: dict) -> dict:
    company = _resolve_or_error(_require_string(args, "company"))
...
TOOL_GET_ACCOUNTING_POLICY_CHANGES = Tool(
    name="get_accounting_policy_changes",
    description=(
        "사업보고서 주석 2/3/4의 회계정책·추정판단 문구 변화를 5개년으로 비교한다. "
        "정책 변경 후보, 추정 불확실성 변화, 전년 대비 문구 변경을 검토할 때 사용한다."
    ),
    inputSchema={
        "type": "object",
        "properties": {
...
def _handle_get_quality_of_earnings_pack(args: dict) -> dict:
    company = _resolve_or_error(_require_string(args, "company"))
...
TOOL_GET_QUALITY_OF_EARNINGS_PACK = Tool(
    name="get_quality_of_earnings_pack",
    description=(
        "투자자 관점에서 5개년 이익의 질을 점검한다. 순이익-영업현금흐름 전환율, 영업마진 변동성, "
        "음의 영업현금흐름, 감사보고서 matter 신호를 함께 반환한다."
    ),
    inputSchema={
        "type": "object",
        "properties": {
