...
def test_note_enrichment_marks_empty_cache_missing_without_claiming_filing_absence():
    ...
    assert "로컬 캐시" in result["data_quality"]["coverage_note"]
    assert "뜻하지 않습니다" in result["data_quality"]["coverage_note"]
...
def test_note_pack_and_chatbot_table_share_enriched_status_and_evidence():
    ...
    assert table["rows"] == [{
        ...
        "confirmed_statement": "주석 7 재고자산에 재고자산 관련 문구가 기재되어 있습니다.",
    ...
    assert text.index("확인된 내용") < text.index("시각화 대체 표")
