...
def test_search_answer_leads_with_actual_expression_and_source_text(
    ...
    from kreports.mcp.chatbot_integration import enrich_chatbot_response
    ...
    assert record["cached_rcept_no"] == "20250318000001"
    assert record["rcept_no"] is None
    assert "source_url" not in record

    enriched = enrich_chatbot_response(
        "search_dataset",
        raw,
    )
    ...
    assert action["filing"]["url"] is None
    assert action["filing"]["receiptNumber"] is None
