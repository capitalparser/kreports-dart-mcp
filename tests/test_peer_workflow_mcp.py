...
def test_auditor_decision_wrappers_forward_a_custom_peer_group(monkeypatch):
    from kreports.analysis import auditor_decisions

    peer_group = {"subject": {"corp_code": "00000001"}, "peers": []}
    captured = {}
    ...
    def fake_matters(**kwargs):
        captured["matters"] = kwargs
        return {"error": "fixture"}
    ...
    assert auditor_decisions.compare_peer_kam_topics(
        "00000001", _peer_group=peer_group
    ) == {"error": "fixture"}
    assert auditor_decisions.compare_peer_audit_report_matters(
        "00000001", _peer_group=peer_group
    ) == {"error": "fixture"}
    assert captured["kam"]["_peer_group"] is peer_group
    assert captured["matters"]["_peer_group"] is peer_group
